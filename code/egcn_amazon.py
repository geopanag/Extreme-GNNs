...
if __name__ == '__main__':
    ...
    for ri in range(repetitions):
        ...
        for ds in  ['computers','photo']: # Amazon, Coauthor
            break
            if George:
                path = osp.join(osp.dirname( osp.realpath("/home/george/Desktop/extreme-gnns/data")), 'data', ds) #/home/dascim/panago/
            else:
            ...
            if(degree_based):
                degs = dict(g.degree())
                #logw = open("../results_new/logw_gcn_deg_"+str(ri)+"_"+ds+".csv","w")
            else:
                degs = nx.core_number(g)
                #logw = open("../results_new/logw_gcn_core_"+str(ri)+"_"+ds+".csv","w")
            names = list(g.nodes())
